...
public class AuthServiceTest extends ApplicationTestSupport {
    ...
    class Login {
        ...
        void givenLoginDataAndAlreadyHasRefreshToken_whenLogin_thenSuccess() {
            ...
            supportRepository.save(MemberToken.builder()
                    .memberId(1L)
                    .memberToken("token.token.token")
                    .build());

            // when
            authService.login(OAuthProvider.KAKAO, request, "code");

            // then
            assertThat(supportRepository.findById(MemberToken.class, 1L).get().getMemberToken())
